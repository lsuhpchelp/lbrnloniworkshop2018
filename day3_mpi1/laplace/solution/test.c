...
int main( int argc, char **argv ){
  ...
  if ( NR % nprocs != 0 && myrank ==0){
	printf("\nError: the number of rows is not multiple of nprocs!\n");
...
for( iter=1; iter<=NITER; iter++ ) {
  ...
  /* Send my data down to the processor below me; Only nprocs-1 do this */
      if( myrank < nprocs-1 )
       MPI_Send(&t[NRL][1], NC, MPI_FLOAT, myrank+1, DOWN, MPI_COMM_WORLD);

  /* Sending my data up to the processor above me ; Only nprocs-1 do this */
      if( myrank != 0 )
  ...
  /***************************************************************************************/
  /*  Detemermine max value from all processors, followed by update bacl to all processors*/
  ...
  /***************************************************************************************/
       ...
       /* Print some test values */
        if ( myrank == 0 ){
            ...
            if (dtg < ERR){
  	         printf("\nSolution has converged at iteration %d\n\n", iter);
                 MPI_Abort(MPI_COMM_WORLD, stat);
            }
         }
